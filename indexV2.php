...
<body>
    ...
    <div class="container">
        <button class="btn btn-secondary mb-3" onclick="toggleSection('searchSection')">Buscar Placas</button>
        <div id="searchSection" style="display: none;">
            <form action="index.php" method="POST">
                <div class="mb-4">
                    <h5>Buscar Placa</h5>
                    <input type="text" class="form-text" id="busca" name="busca" size="30" maxlength="30" value="<?php echo htmlspecialchars($buscar); ?>">
            ...
            <tbody>
                <?php if ($showSearchResults && $totalRegistros > 0): ?>
                    <?php while($row = mysqli_fetch_assoc($query)): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($row['eqtrans_placa']); ?></td>
                            <td><?php echo htmlspecialchars($row['id_equipo_transporte']); ?></td>
                            <td><?php echo htmlspecialchars($row['cliente_des_oficial']); ?></td>
                            <td><?php echo htmlspecialchars($row['consumo_saldo_actual']); ?></td>
                            <td><?php echo htmlspecialchars($row['consumo_kilometraje_inicial']); ?></td>
                            <td><?php echo htmlspecialchars($row['consumo_kilometraje_final']); ?></td>
                            <td><a href="consumos.php?id_equipo_transporte=<?php echo urlencode($row['id_equipo_transporte']); ?>">Consumos</a></td>
                        </tr>
                    <?php endwhile; ?>
    ...
    <!-- Todos los registros -->
    <div class="container" id="allRecords" style="<?php echo !$showSearchResults ? 'display: block;' : 'display: none;'; ?>">
